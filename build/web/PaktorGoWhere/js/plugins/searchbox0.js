...
$(function() {
    ...
    $('#searchTextbox').keyup(function() {
        ...
        if ($('#searchTextbox').val().length > 1) {
            ...
            // Case insensitive
            $('#list-table tr td:contains(\'' + convertedInput + '\')').parent("tr").show();
            $('#list-table tr td:contains(\'' + convertToCourseCase + '\')').parent("tr").show();
            $('#list-table tr td:contains(\'' + convertToUpperCase + '\')').parent("tr").show();
            // Case sensitive
            $('#list-table tr td:contains(\'' + input + '\')').parent("tr").show();
        ...
        if ($('#list-table tr:visible').length == 0) {
            // remove the norecords row if it already exists
            $('.norecords').remove();
            // add the norecords row
            $('#list-table').append('<tr class="norecords"><td colspan="3" class="text-center">No records were found</td></tr>');
        }
